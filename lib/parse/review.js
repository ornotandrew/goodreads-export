...
function reviewIds(jsText) {
    ...
    var _a = [match[1], match[2]].map(function (n) { return parseInt(n); }), current = _a[0], total = _a[1];
    ...
    return { reviewIds: reviewIds, progress: { current: current, total: total }, isLastPage: isLastPage };
}
...
function review(html) {
    ...
    var rows = rowText
        ...
        .replace(/ +/g, ' ')
        .split('\n')
        .filter(function (s) { return !s.includes('Shelved as:'); })
        .filter(function (s) { return s.includes('-'); }) // e.g. "Started Reading" with no date
    ...
    var updates = rows.reduce(function (acc, row) {
        var _a;
    ...
    var bookPath = ast('a.bookTitle[itemprop="url"]').attr('href');
    return {
